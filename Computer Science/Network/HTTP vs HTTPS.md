...
- 데이터 암호화로 인해 중간자 공격 및 도청 방지.
- 데이터 무결성을 보장하여 전송 중 변조 방지.

2. **신뢰성 확보**:

- 인증서를 통해 서버의 신원을 보장.
- 브라우저에서 HTTPS 사용 사이트는 “안전” 표시.

3. **검색 엔진 최적화(SEO)**:

- Google은 HTTPS를 사용하는 웹사이트를 우선적으로 노출.

4. **브라우저 경고 방지**:

- HTTPS를 사용하지 않으면 브라우저에서 “안전하지 않음” 경고 표시.

## HTTPS 통신 흐름

![img/https.png](img/https.png)
